...
const vehicleData = [
  { brand: 'Chevrolet', model: 'Onix Plus LTZ',     year: '2023', category: 'seminovo' },
  { brand: 'Hyundai',   model: 'HB20 Vision',        year: '2022', category: 'seminovo' },
  { brand: 'Toyota',    model: 'Hilux SRX 4x4',      year: '2023', category: 'pickup'   },
  { brand: 'Jeep',      model: 'Renegade Longitude',  year: '2022', category: 'suv'      },
  { brand: 'Fiat',      model: 'Strada Volcano',      year: '2024', category: 'pickup'   },
  { brand: 'Volkswagen',model: 'Polo Track',          year: '2024', category: 'novo'     },
]
...
export default function App() {
  ...
  const [calcParcelas, setCalcParcelas] = useState(60)
  ...
  const [lojaStatus, setLojaStatus] = useState('Verificando...')
  const [resultInfo, setResultInfo] = useState({ visible: false, text: '' })
  ...
  useEffect(() => {
    ...
    let aberto = false
    let horarioHoje = ''
    if (dia === 0) { horarioHoje = 'Fechado' }
    else if (dia === 1) { horarioHoje = '07:30 – 18:00'; aberto = hora >= 7.5 && hora < 18 }
    else if (dia === 6) { horarioHoje = '08:00 – 12:00'; aberto = hora >= 8 && hora < 12 }
    else { horarioHoje = '07:30 – 17:00'; aberto = hora >= 7.5 && hora < 17 }
    if (aberto) {
      setLojaStatus(`<span style="color:#4ade80">● Aberto agora</span><br><span style="font-size:12px;font-weight:400;color:var(--text-muted)">${horarioHoje}</span>`)
    } else {
      setLojaStatus(`<span style="color:#f87171">● Fechado no momento</span><br><span style="font-size:12px;font-weight:400;color:var(--text-muted)">${horarioHoje}</span>`)
    }
  }, [])
  ...
  const applyFilters = (search = searchVal, brand = currentBrand, category = currentCategory) => {
    ...
    cards.forEach(card => {
      ...
      if (show) { visibleCount++; card.style.opacity = '1'; card.style.transform = ''; card.style.display = '' }
      else { card.style.opacity = '0'; card.style.transform = 'scale(0.95)'; setTimeout(() => { card.style.display = 'none' }, 300) }
    })
    const q = search.trim().toLowerCase()
    if (q || brand !== 'todas') {
      const label = q ? `"${q}"` : `Marca: ${brand.charAt(0).toUpperCase() + brand.slice(1)}`
      setResultInfo({ visible: true, text: `<strong>${visibleCount}</strong> veículo${visibleCount !== 1 ? 's' : ''} encontrado${visibleCount !== 1 ? 's' : ''} para ${label}` })
  ...
  const handleSearchInput = (val) => {
    ...
    if (!q) { setSuggestions([]); setShowSuggestions(false) }
    else {
      const matches = vehicleData.filter(v => v.brand.toLowerCase().includes(q) || v.model.toLowerCase().includes(q))
  ...
  useEffect(() => {
    ...
    const parcelas = parseInt(calcParcelas) || 60
    if (valor <= 0 || entrada >= valor) { setCalcResult('R$ --'); setCalcNote('Preencha os campos acima para simular'); return }
    const taxa = 0.0169
  ...
  const submitForm = (e) => {
    ...
    window.open(`https://wa.me/5535999091119?text=${encodeURIComponent(text)}`, '_blank')
  }
  ...
  const highlightText = (text, query) => {
    ...
    return parts.map((part, i) =>
      part.toLowerCase() === query.toLowerCase() ? <em key={i} style={{ color: 'var(--gold)', fontStyle: 'normal' }}>{part}</em> : part
    )
  }
    ...
    <>
      ...
      <nav id="navbar" className={scrolled ? 'scrolled' : ''}>
        <a href="#hero" className="nav-logo" id="nav-logo-link">
          <img src="/assets/images/logo.png" alt="JB Multimarcas" className="nav-logo-img" id="nav-logo-img" />
        </a>
        ...
        <div className="hero-content">
          ...
          <div className="hero-stats">
            <div className="hero-stat-item">
              <div className="hero-stat-number">Avaliação</div>
              <div className="hero-stat-label">5⭐</div>
            </div>
            <div className="hero-stat-item">
          ...
          <div className="hero-buttons">
            <a href="#catalogo" className="btn-primary" id="hero-ver-catalogo">
              <i className="fas fa-car"></i>
              Ver Catálogo
            </a>
            <a href="https://wa.me/5535999091119?text=Olá!%20Vim%20pelo%20site%20e%20gostaria%20de%20mais%20informações."
              target="_blank" rel="noreferrer" className="btn-secondary" id="hero-whatsapp">
        ...
        <div className="services-strip-inner">
          {[
            ...
            <div className="service-strip-item" key={i}>
              ...
              <div className="service-strip-text">
      ...
      <section id="catalogo">
        ...
        <div className="catalog-search-wrapper fade-in-up">
          ...
          <div className={`catalog-suggestions${showSuggestions ? ' open' : ''}`} id="catalogSuggestions">
            {suggestions.map((v, i) => {
              ...
              return (
                <div key={i} className="suggestion-item" onMouseDown={() => selectSuggestion(`${v.brand} ${v.model}`)}>
                  ...
                  <div className="suggestion-item-text">
                    ...
                    <span>{v.year} · {v.category === 'novo' ? '0KM' : v.category.charAt(0).toUpperCase() + v.category.slice(1)}</span>
                  </div>
                </div>
              )
            })}
        ...
        {/* Brand Pills */}
        <div className="catalog-brand-pills fade-in-up" id="brandPills">
          {[
            { brand: 'todas', label: 'Todas as Marcas', icon: 'fa-th' },
            { brand: 'chevrolet', label: 'Chevrolet', icon: 'fa-car' },
            { brand: 'hyundai', label: 'Hyundai', icon: 'fa-car' },
            { brand: 'toyota', label: 'Toyota', icon: 'fa-car' },
            { brand: 'jeep', label: 'Jeep', icon: 'fa-car' },
            { brand: 'fiat', label: 'Fiat', icon: 'fa-car' },
            { brand: 'volkswagen', label: 'Volkswagen', icon: 'fa-car' },
          ].map(item => (
            <button key={item.brand} className={`brand-pill${currentBrand === item.brand ? ' active' : ''}`} data-brand={item.brand} onClick={() => filterByBrand(item.brand)}>
        ...
        {/* Result info bar */}
        <div className={`catalog-result-info${resultInfo.visible ? ' visible' : ''}`} id="catalogResultInfo">
          <i className="fas fa-filter"></i>
          <span id="resultInfoText" dangerouslySetInnerHTML={{ __html: resultInfo.text }}></span>
        ...
        <div className="catalog-filters fade-in-up">
          {[
            ...
            <button key={item.key} className={`filter-btn${currentCategory === item.key ? ' active' : ''}`} id={`filter-${item.key}`} onClick={() => filterVehicles(item.key)}>{item.label}</button>
          ))}
        </div>

        <div className="vehicles-grid" id="vehiclesGrid">
          {/* No results placeholder */}
          <div className={`catalog-no-results${noResults ? ' visible' : ''}`} id="catalogNoResults">
            ...
            <a href="https://wa.me/5535999091119?text=Olá!%20Estou%20buscando%20um%20veículo%20específico%20e%20gostaria%20de%20ajuda." target="_blank" rel="noreferrer" className="btn-primary" style={{ display: 'inline-flex' }}>
              <i className="fab fa-whatsapp"></i> Perguntar pelo WhatsApp
            </a>
          </div>

          {/* Onix Plus */}
          <div className="vehicle-card fade-in-up" data-category="seminovo" data-brand="chevrolet" data-model="onix plus ltz">
            <div className="vehicle-card-image">
              <img src="/assets/images/car_onix.png" alt="Chevrolet Onix Plus 2023" loading="lazy" />
              <span className="vehicle-badge badge-seminovo">Seminovo</span>
            </div>
            <div className="vehicle-card-content">
              <div className="vehicle-brand">Chevrolet</div>
              <div className="vehicle-name">Onix Plus LTZ</div>
              <div className="vehicle-year">2023 · 18.500 km</div>
              <div className="vehicle-specs">
                <span className="vehicle-spec"><i className="fas fa-gas-pump"></i> Flex</span>
                <span className="vehicle-spec"><i className="fas fa-cog"></i> Automático</span>
                <span className="vehicle-spec"><i className="fas fa-palette"></i> Prata</span>
              </div>
              <div className="vehicle-card-footer">
                <a href="https://wa.me/5535999091119?text=Olá!%20Tenho%20interesse%20no%20Onix%20Plus%202023." target="_blank" rel="noreferrer" className="vehicle-btn" id="btn-onix">
          ...
          {/* HB20 */}
          <div className="vehicle-card fade-in-up" data-category="seminovo" data-brand="hyundai" data-model="hb20 vision">
            <div className="vehicle-card-image">
              <img src="/assets/images/car_hb20.png" alt="Hyundai HB20 2022" loading="lazy" />
              <span className="vehicle-badge badge-seminovo">Seminovo</span>
            </div>
            <div className="vehicle-card-content">
              <div className="vehicle-brand">Hyundai</div>
              <div className="vehicle-name">HB20 Vision</div>
              <div className="vehicle-year">2022 · 32.000 km</div>
              <div className="vehicle-specs">
                <span className="vehicle-spec"><i className="fas fa-gas-pump"></i> Flex</span>
                <span className="vehicle-spec"><i className="fas fa-cog"></i> Manual</span>
                <span className="vehicle-spec"><i className="fas fa-palette"></i> Vermelho</span>
              </div>
              <div className="vehicle-card-footer">
                <a href="https://wa.me/5535999091119?text=Olá!%20Tenho%20interesse%20no%20HB20%202022." target="_blank" rel="noreferrer" className="vehicle-btn" id="btn-hb20">
                  <i className="fab fa-whatsapp"></i> Tenho Interesse
                </a>
              </div>
            </div>
          ...
          {/* Hilux */}
          <div className="vehicle-card fade-in-up" data-category="pickup" data-brand="toyota" data-model="hilux srx 4x4">
            <div className="vehicle-card-image">
              <img src="/assets/images/car_hilux.png" alt="Toyota Hilux 2023" loading="lazy" />
              <span className="vehicle-badge badge-seminovo">Seminovo</span>
            </div>
            <div className="vehicle-card-content">
              <div className="vehicle-brand">Toyota</div>
              <div className="vehicle-name">Hilux SRX 4x4</div>
              <div className="vehicle-year">2023 · 25.000 km</div>
              <div className="vehicle-specs">
                <span className="vehicle-spec"><i className="fas fa-gas-pump"></i> Diesel</span>
                <span className="vehicle-spec"><i className="fas fa-cog"></i> Automático</span>
                <span className="vehicle-spec"><i className="fas fa-palette"></i> Branca</span>
              </div>
              <div className="vehicle-card-footer">
                <a href="https://wa.me/5535999091119?text=Olá!%20Tenho%20interesse%20na%20Hilux%202023." target="_blank" rel="noreferrer" className="vehicle-btn" id="btn-hilux">
                  <i className="fab fa-whatsapp"></i> Tenho Interesse
                </a>
              </div>
            </div>
          ...
          {/* Renegade */}
          <div className="vehicle-card fade-in-up" data-category="suv" data-brand="jeep" data-model="renegade longitude">
            <div className="vehicle-card-image">
              <img src="/assets/images/car_renegade.png" alt="Jeep Renegade 2022" loading="lazy" />
              <span className="vehicle-badge badge-seminovo">Seminovo</span>
            </div>
            <div className="vehicle-card-content">
              <div className="vehicle-brand">Jeep</div>
              <div className="vehicle-name">Renegade Longitude</div>
              <div className="vehicle-year">2022 · 41.000 km</div>
              <div className="vehicle-specs">
                <span className="vehicle-spec"><i className="fas fa-gas-pump"></i> Flex</span>
                <span className="vehicle-spec"><i className="fas fa-cog"></i> Automático</span>
                <span className="vehicle-spec"><i className="fas fa-palette"></i> Azul</span>
              </div>
              <div className="vehicle-card-footer">
                <a href="https://wa.me/5535999091119?text=Olá!%20Tenho%20interesse%20no%20Renegade%202022." target="_blank" rel="noreferrer" className="vehicle-btn" id="btn-renegade">
                  <i className="fab fa-whatsapp"></i> Tenho Interesse
                </a>
              </div>
            </div>
          ...
          {/* Strada */}
          <div className="vehicle-card fade-in-up" data-category="novo pickup" data-brand="fiat" data-model="strada volcano">
            <div className="vehicle-card-image">
              <div style={{ width: '100%', height: '100%', background: 'linear-gradient(135deg,#0a0a18,#141426)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <i className="fas fa-truck-pickup" style={{ fontSize: '80px', color: 'rgba(255,197,1,0.2)' }}></i>
              </div>
              <span className="vehicle-badge badge-novo">0KM</span>
            </div>
            <div className="vehicle-card-content">
              <div className="vehicle-brand">Fiat</div>
              <div className="vehicle-name">Strada Volcano</div>
              <div className="vehicle-year">2024 · 0 km · Novo</div>
              <div className="vehicle-specs">
                <span className="vehicle-spec"><i className="fas fa-gas-pump"></i> Flex</span>
                <span className="vehicle-spec"><i className="fas fa-cog"></i> CVT</span>
                <span className="vehicle-spec"><i className="fas fa-palette"></i> Cinza</span>
              </div>
              <div className="vehicle-card-footer">
                <a href="https://wa.me/5535999091119?text=Olá!%20Tenho%20interesse%20na%20Fiat%20Strada%20Volcano%200KM." target="_blank" rel="noreferrer" className="vehicle-btn" id="btn-strada">
          ...
          {/* Polo Track */}
          <div className="vehicle-card fade-in-up" data-category="novo" data-brand="volkswagen" data-model="polo track">
            <div className="vehicle-card-image">
              <div style={{ width: '100%', height: '100%', background: 'linear-gradient(135deg,#0a0a18,#141426)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <i className="fas fa-car" style={{ fontSize: '80px', color: 'rgba(255,197,1,0.2)' }}></i>
              </div>
              <span className="vehicle-badge badge-novo">0KM</span>
            </div>
            <div className="vehicle-card-content">
              <div className="vehicle-brand">Volkswagen</div>
              <div className="vehicle-name">Polo Track</div>
              <div className="vehicle-year">2024 · 0 km · Novo</div>
              <div className="vehicle-specs">
                <span className="vehicle-spec"><i className="fas fa-gas-pump"></i> Flex</span>
                <span className="vehicle-spec"><i className="fas fa-cog"></i> Automático</span>
                <span className="vehicle-spec"><i className="fas fa-palette"></i> Preto</span>
              </div>
              <div className="vehicle-card-footer">
                <a href="https://wa.me/5535999091119?text=Olá!%20Tenho%20interesse%20no%20VW%20Polo%20Track%200KM." target="_blank" rel="noreferrer" className="vehicle-btn" id="btn-polo">
                  <i className="fab fa-whatsapp"></i> Consultar
                </a>
              </div>
            </div>
        ...
        <div className="catalog-cta-wrapper fade-in-up">
          <a href="https://wa.me/5535999091119?text=Olá!%20Gostaria%20de%20ver%20todos%20os%20veículos%20disponíveis."
            target="_blank" rel="noreferrer" className="btn-primary" id="catalog-ver-todos">
            <i className="fab fa-whatsapp"></i>
      ...
      <section id="sobre">
        <div className="sobre-grid">
          <div className="sobre-image-wrapper fade-in-up">
            <div className="sobre-card-float2">
              <i className="fas fa-handshake icon-big"></i>
            </div>
          ...
          <div className="sobre-content fade-in-up">
            ...
            <p>
              A JB Multimarcas surgiu da iniciativa de dois amigos, <strong style={{ color: 'var(--text-primary)' }}>Júnior e Bruno</strong>. Em 2020, por acaso, venderam juntos o primeiro carro — e isso se tornou recorrente.
            </p>
            <p>
              Em julho de 2021, criaram o Instagram para ampliar o alcance do negócio, batizando-o de JB Multimarcas. Em fevereiro de 2024, finalmente tiraram do papel o projeto mais esperado: <strong style={{ color: 'var(--gold)' }}>a loja física!</strong>
            </p>
            <p>
              Hoje, nossa loja em Cachoeira de Minas é o lugar ideal para você garantir seu veículo com segurança, conforto e, claro, um delicioso cafezinho! ☕
            </p>
        ...
        <div className="financiamento-grid">
          <div className="calculator-card fade-in-up">
            ...
            <div className="result-card">
              ...
              <div className="result-value" id="calc-result">{calcResult}</div>
              <div className="result-note" id="calc-note">{calcNote}</div>
            </div>
            <a href="https://wa.me/5535999091119?text=Olá!%20Quero%20fazer%20uma%20simulação%20de%20financiamento."
              target="_blank" rel="noreferrer" className="btn-primary" style={{ width: '100%', justifyContent: 'center', marginTop: '20px' }}
              id="fin-whatsapp">
              <i className="fab fa-whatsapp"></i>
            ...
            <form id="contactForm" onSubmit={submitForm}>
              ...
              <div className="form-group">
                ...
                <select className="form-control" id="contact-interesse" value={formData.interesse} onChange={e => setFormData(f => ({ ...f, interesse: e.target.value }))}>
                  ...
                  <option>Chevrolet Onix Plus 2023</option>
                  <option>Hyundai HB20 2022</option>
                  <option>Toyota Hilux 2023</option>
                  <option>Jeep Renegade 2022</option>
                  <option>Fiat Strada Volcano 2024</option>
                  <option>Volkswagen Polo Track 2024</option>
              ...
              <button type="submit" className="btn-submit" id="btn-enviar-form">
                <i className="fab fa-whatsapp"></i>
                Enviar via WhatsApp
              </button>
            </form>
        ...
        <div className="footer-grid">
          <div className="footer-brand">
            ...
            <div className="footer-social">
              <a href="https://wa.me/5535999091119" target="_blank" rel="noreferrer" className="social-btn" id="footer-whatsapp" title="WhatsApp"><i className="fab fa-whatsapp"></i></a>
              <a href="https://instagram.com/jb.multimarcaas" target="_blank" rel="noreferrer" className="social-btn" id="footer-instagram" title="Instagram"><i className="fab fa-instagram"></i></a>
              <a href="https://g.co/kgs/JBMultimarcas" target="_blank" rel="noreferrer" className="social-btn" id="footer-google" title="Google"><i className="fab fa-google"></i></a>
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <ul className="footer-links">
              <li><a href="https://wa.me/5535999091119" target="_blank" rel="noreferrer">(35) 99909-1119</a></li>
              <li><a href="https://instagram.com/jb.multimarcaas" target="_blank" rel="noreferrer">@jb.multimarcaas</a></li>
              <li><a href="#contato">Cachoeira de Minas - MG</a></li>
            </ul>
            <div style={{ marginTop: '24px', padding: '16px', background: 'rgba(255,197,1,0.08)', border: '1px solid rgba(255,197,1,0.2)', borderRadius: '12px' }}>
              <div style={{ fontSize: '11px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px' }}>Aberto Hoje</div>
              <div id="statusLoja" style={{ fontFamily: "'Outfit',sans-serif", fontSize: '15px', fontWeight: 700, color: 'var(--gold)' }} dangerouslySetInnerHTML={{ __html: lojaStatus }}></div>
            </div>
          </div>
        </div>
        <div className="footer-bottom">
          <p>© 2024 <span>JB Multimarcas</span>. Todos os direitos reservados.</p>
          <p>Desenvolvido com <span>♥</span> para o melhor atendimento de Cachoeira de Minas</p>
        </div>
      </footer>

      {/* ===== WHATSAPP FLOAT ===== */}
      <div id="whatsapp-float">
        <a href="https://wa.me/5535999091119?text=Olá!%20Vim%20pelo%20site%20da%20JB%20Multimarcas%20e%20gostaria%20de%20mais%20informações."
          target="_blank" rel="noreferrer" className="whatsapp-btn" id="float-whatsapp">
          <i className="fab fa-whatsapp"></i>
